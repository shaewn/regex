...
struct Parser {
    ...
    Parser(const char *text, ParseTree *parse_tree)
        : pointer(text), parse_tree(parse_tree), nodes(parse_tree->nodes) {}
    ...
    bool parse() { return parse_alternation(); }

    bool parse_alternation() {
        if (!parse_concatenation())
            return false;
    ...
    bool parse_unit() {
        if (isalpha(tolower(*pointer)) || isdigit(*pointer) ||
            *pointer == '_') {
            Node &node = nodes.emplace_back();
            node.kind = Node::ATOM;
            node.atom = *pointer++;
    ...
    void debug_print_states() {
        ...
        for (auto &state : states) {
            ...
            for (auto &t : state.transitions) {
                std::cerr << "Transitions for "
                          << (t.symbol == -1 ? '!' : (char)t.symbol) << " -> ";
...
struct NFACompiler {
    ...
    void emit_node(const std::vector<Node> &nodes, uint32_t node) {
        ...
        case Node::ATOM: {
            ...
            auto &t =
                machine->states[my_data.start_state].get(nodes[node].atom);
...
struct DFAConverter {
    DFA *machine;
    typedef std::vector<uint32_t> StateList;

    void convert_nfa(NFA *nfa) {

        std::vector<StateList> state_list_queue;

        StateList &d0 = state_list_queue.emplace_back();
        d0.push_back(nfa->start_state);
        epsilon_closure(nfa, d0);

        while (!state_list_queue.empty()) {
        }
    }

    void epsilon_closure(NFA *nfa, StateList &state_list) {
        std::vector<uint32_t> list;
        for (uint32_t i = 0; i < state_list.size(); i++) {
            list.resize(0);
            list.push_back(state_list[i]);

            for (uint32_t j = 0; j < list.size(); j++) {
                uint32_t state = list[j];

                NFA::Transition *t = nfa->states[state].find(-1);

                if (t) {
                    for (uint32_t future : t->futures) {
                        if (std::find(list.begin(), list.end(), future) ==
                            list.end()) {
                            list.push_back(future);
                        }
                    }
                } else {
                    if (std::find(state_list.begin(), state_list.end(),
...
struct NFAEngine {
    ...
    void step() {
        ...
        for (uint32_t i = 0; i < cur_states->size(); i++) {
            ...
            if (t) {
                cur_states->insert(cur_states->end(), t->futures.begin(),
                                   t->futures.end());
            }
...
int main() {
    ...
    std::cout << "Parsing status: "
              << (parser.build_parse_tree() ? "success" : "FAILURE") << '\n';
    ...
    std::cout << "Execution status: "
              << (engine.execute(test_string) ? "matches" : "DOESN'T MATCH")
              << '\n';
}
